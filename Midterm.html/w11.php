...
<head>
    ...
    <style>
        ...
        .star-output { line-height: 1.4; letter-spacing: 0.1em; font-family: 'Courier New', Courier, monospace; }
...
<body class="bg-gradient-to-br from-slate-50 to-gray-100 min-h-screen flex flex-col">
    ...
    <nav class="bg-[#0f172a]/90 backdrop-blur-md text-white py-4 px-6 flex justify-between items-center shadow-md sticky top-0 z-50 border-b border-white/10">
        <div class="container mx-auto flex justify-between items-center">
            <div class="flex items-center gap-3 cursor-pointer hover:opacity-80 transition">
                <div class="bg-gradient-to-br from-fuchsia-500 to-pink-600 p-1.5 rounded-lg shadow-inner">
            ...
            <div class="hidden md:flex gap-8 text-sm font-medium items-center">
                ...
                <a href="#" class="text-fuchsia-400 font-semibold border-b-2 border-fuchsia-400 pb-1">เขียนโปรแกรมวนซ้ำ ▾</a>
            </div>
        </div>
    </nav>

    <main class="flex-grow container mx-auto flex justify-center items-center py-12 px-4 relative">
        <div class="absolute top-10 right-20 w-72 h-72 bg-fuchsia-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 pointer-events-none hidden md:block"></div>
        ...
        <div class="max-w-4xl w-full bg-white/95 backdrop-blur-xl rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col md:flex-row border border-gray-100 relative z-10 min-h-[420px]">
            ...
            <div class="flex-1 p-8 md:p-12 relative flex flex-col justify-center">
                <h2 class="text-3xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-gray-800 to-gray-600 flex items-center gap-3">
                    <span class="bg-gray-100 text-gray-700 w-10 h-10 flex items-center justify-center rounded-xl shadow-sm text-lg">2.5</span>
                    รูปดาวแบบพีระมิด
                </h2>
                
                <div class="space-y-6 relative z-10">
                    <div class="group">
                        <label class="block text-sm font-semibold text-gray-600 mb-1.5 group-focus-within:text-fuchsia-600 transition-colors">กรอกจำนวนแถว (n)</label>
                        <input type="number" id="rowInput" placeholder="กรอกค่า n" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-gray-700 focus:bg-white focus:ring-4 focus:ring-fuchsia-500/10 focus:border-fuchsia-500 transition-all outline-none shadow-sm hover:border-fuchsia-300 text-lg">
                    </div>

                    <div class="flex flex-col sm:flex-row gap-4 mt-8">
                        <button onclick="generatePyramid()" class="flex-[1.5] bg-gradient-to-r from-fuchsia-500 to-pink-500 hover:from-fuchsia-600 hover:to-pink-600 text-white font-bold py-3.5 px-6 rounded-xl shadow-lg shadow-fuchsia-500/30 transform active:scale-95 transition-all duration-200">
                            แสดงรูปแบบ
                        </button>
                        <button onclick="clearAll()" class="flex-1 bg-white border-2 border-gray-200 hover:bg-gray-50 hover:border-gray-300 text-gray-600 font-bold py-3.5 px-6 rounded-xl transform active:scale-95 transition-all duration-200">
                            ล้างข้อมูล
                        </button>
                    </div>

                    <div id="resultArea" class="mt-8 p-6 bg-gradient-to-b from-fuchsia-50 to-pink-50 rounded-2xl border border-fuchsia-100 shadow-inner hidden animate-fade-up overflow-x-auto text-center">
                        <pre id="starOutput" class="star-output text-center text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 to-pink-500 drop-shadow-sm inline-block"></pre>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 bg-gradient-to-br from-[#ff7675] to-[#fab1a0] p-8 md:p-12 text-white flex flex-col justify-center relative overflow-hidden">
                ...
                <div class="relative z-10 text-center md:text-left">
                    ...
                    <p class="mb-8 text-white/90 font-light leading-relaxed">
                        กรอกจำนวนแถวที่ต้องการ แล้วโปรแกรมจะแสดงรูปแบบ <strong>ดาวแบบพีระมิดสมมาตร</strong>
                    </p>
                    ...
                    <div class="space-y-4 text-sm opacity-95 text-left">
                        <section class="bg-white/10 p-5 rounded-2xl backdrop-blur-md border border-white/20 shadow-lg">
                            ...
                            <ul class="space-y-3 list-none ml-0">
                                <li class="flex gap-2 items-start"><span class="text-lg">🔁</span> ใช้ Loop ซ้อนเพื่อตรวจสอบการเว้นวรรคและเครื่องหมายดาว</li>
                                <li class="flex gap-2 items-start"><span class="text-lg">⭐</span> แถวแรกมี 1 ดาว, แถวถัดไปเพิ่มทีละ 2 (1, 3, 5...) จนถึงแถวที่ n</li>
                            </ul>
                        </section>
                    </div>
                </div>
    ...
    <script>
        function generatePyramid() {
            ...
            let result = "";
            for (let i = 1; i <= n; i++) {
                // ส่วนของเว้นวรรค (Spaces)
                let spaces = " ".repeat(n - i);
                // ส่วนของดาว (Stars) - เพิ่มทีละ 2 ดวง (1, 3, 5, ...)
